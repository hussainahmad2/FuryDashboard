...
class FuryHistoricWeekly extends StatefulWidget {
  const FuryHistoricWeekly({super.key});

  @override
  State<FuryHistoricWeekly> createState() => _FuryHistoricWeeklyState();
}

class _FuryHistoricWeeklyState extends State<FuryHistoricWeekly> {
  List<Map<String, dynamic>> weeklyData = [];
  final ScrollController _horizontalScrollController = ScrollController();
  final ScrollController _verticalScrollController = ScrollController();
  final ScrollController _weekColumnScrollController = ScrollController();
  bool isLoading = true;
  String? errorMessage;
  ...
  void dispose() {
    ...
    _weekColumnScrollController.dispose();
    super.dispose();
  }
    ...
    try {
      ...
      for (var row in rows) {
        try {
          ...
          String weekKey = DateFormat('MMM dd').format(weekStart);

          weeklyGroups.putIfAbsent(weekKey, () => []).add(map);
        } catch (e) {
      ...
      weeklyGroups.forEach((weekKey, dailyData) {
        try {
          ...
          debugPrint('Error calculating weekly aggregates for $weekKey: $e');
        }
      });
      ...
      result.sort((a, b) {
        try {
          DateTime dateA = DateFormat('MMM dd').parse(a['Week']);
          DateTime dateB = DateFormat('MMM dd').parse(b['Week']);
